...
def add_watched_movie(movie_id):
    ...
    for user in participants:
        watched_movie = WatchedMovie.query.filter(WatchedMovie.user_mail == user).first()
        if not watched_movie:
            watched_movie = WatchedMovie(movie_id=movie_id, user_mail=user, watch_num=1, date_modify=date)
        else:
