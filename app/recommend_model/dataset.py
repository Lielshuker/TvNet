...
class Dataset:
    ...
    def load_or_generate_data(self, update_to_latest_db=True):
        ...
        self.train = tf.data.experimental.make_csv_dataset(os.path.join(dirname, RATING_TRAIN_FILENAME),
        ...
        self.test = tf.data.experimental.make_csv_dataset(os.path.join(dirname, RATING_TEST_FILENAME),
                                                          batch_size=self.batch_size, num_epochs=1,
                                                          column_names=['userid', 'movieid',
                                                                        'rating'])  # column_defaults=[tf.string, tf.constant(float('nan'), tf.float32)])
        self.movies = tf.data.experimental.make_csv_dataset(os.path.join(dirname, MOVIE_FILENAME),
                                                            batch_size=self.batch_size, num_epochs=1, shuffle=False,
                                                            column_names=['movieid', 'movielensid', 'imdbid', 'title',
                                                                          'cover', 'plot', 'year'
...
def _load_or_generate_csv_data(test_fraction, update_to_latest_db):
    ...
    list_of_dirs = [os.path.join(DATA_PATH, d) for d in os.listdir(DATA_PATH) if
